...
export function MultiSelect<Option>({
  ...
  selectPlaceholder,
  searchPlaceholder,
  noSeachResultMessage = "No results",
} : {
    options: Option[],
    ...
    <Popover open={open} onOpenChange={setOpen}>
          ...
          <div className="flex gap-1 flex-wrap">
                {selectedValues.length > 0 ? (
                    selectedValues.map((value) => { 
                        ...
                        return <Badge key={getValue(option)} variant="outline">
                            {getLabel(option)}
                        </Badge>
                    })
                ) : (
      ...
      <PopoverContent className="w-[200px] p-0">
        <Command>
          ...
          <CommandList>
            <CommandEmpty>{noSeachResultMessage}</CommandEmpty>
            <CommandGroup>
              {options.map((option) => (
                <CommandItem
                  key={getValue(option)}
                  ...
                  onSelect={(currentValue) => {
                    if(selectedValues.includes(currentValue)){
                        ...
                        return onSelectedValuesChange([...selectedValues, currentValue])
                    }
                  }}
                >
                  <Check
